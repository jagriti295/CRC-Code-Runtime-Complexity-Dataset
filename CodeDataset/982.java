...
public class Main implements Runnable {
    ...
    void solve() {
        ...
        printf(calc(dp, a, 1, n));
    }

    int calc(int[][] dp, int[] a, int l, int r) {
        if (l > r) return 0;
        if (dp[l][r] != 0) return dp[l][r];
        if (l == r) return dp[l][r] = 1;
        if (l + 1 == r) return dp[l][r] = a[l] == a[r] ? 1 : 2;
        dp[l][r] = a[l] == a[r] ? calc(dp, a, l + 1, r - 1) : MI;
        for (int i = l; i < r; i++) {
            dp[l][r] = Math.min(dp[l][r], calc(dp, a, l, i) + calc(dp, a, i + 1, r));
        }
        return dp[l][r];
    }
